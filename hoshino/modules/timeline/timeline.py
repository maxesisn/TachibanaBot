...
async def search_dbname(bot, ev: CQEvent):
    if not priv.check_priv(ev, priv.ADMIN):
        await bot.send(ev, '[错误]抱歉，您无权使用此指令，请联系管理员')
        return
    else:
        db = TLDBNameDao(ev.group_id)
        await bot.send(ev, '[信息]当前轴库名为: ' + db._find_by_id(ev.group_id))
...
async def modify_dbname(bot, ev: CQEvent):
    if not priv.check_priv(ev, priv.ADMIN):
        ...
        if s=='':
            await bot.send(ev, '[错误]修改轴库名失败，名称不能为空') 
        else:
            db = TLDBNameDao(ev.group_id)
            db._update_by_id(ev.group_id, ev.message.extract_plain_text())
            await bot.send(ev, '[信息]修改轴库名成功')  
...
async def approve_timeline(bot, ev: CQEvent):
    try:
        ...
        if not priv.check_priv(ev, priv.ADMIN) and len(s)==2 and abs(int(s[1]))!=1:
            await bot.send(ev, '[错误]赞同失败，非管理员单次赞同数只能为1或-1')
            return
        ...
        await bot.send(ev, '[错误]赞同失败，请输入\"帮助pcr轴\"查看指令的使用方式')
...
async def insert_timeline(bot, ev: CQEvent):
    try:
        ...
        await bot.send(ev, '[错误]录入轴失败，请输入\"帮助pcr轴\"查看指令的使用方式')
...
async def search_timeline(bot, ev: CQEvent):
    try:
        ...
        await bot.send(ev, '[错误]查找轴失败，请输入\"帮助pcr轴\"查看指令的使用方式')
...
async def update_timeline(bot, ev: CQEvent):
    try:
        ...
        if db._update_by_id(tid2id(s[0]), s[1], s[2], s[3].strip(), userid):
            await bot.send(ev, f'[信息]修改轴{s[0]}成功')
        else:
            await bot.send(ev, '[错误]您非该轴的上传者或管理员，无修改权限')
    except:
        await bot.send(ev, '[错误]修改轴失败，请输入\"帮助pcr轴\"查看指令的使用方式')
...
async def delete_timeline(bot, ev: CQEvent):
    try:
        ...
        if db._del_by_id(tid2id(s), userid):
            await bot.send(ev, f'[信息]删除轴{s}成功')
        else:
            await bot.send(ev, '[错误]您非该轴的上传者或管理员，无删除权限')
    except:
        await bot.send(ev, '[错误]删除轴失败，请输入\"帮助pcr轴\"查看指令的使用方式')
